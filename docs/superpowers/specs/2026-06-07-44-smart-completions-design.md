...
Tracks: [#44](https://github.com/mellonis/machines-demo/issues/44) (the original "member-completion for `movements.*`, `symbolCommands.*`" ask, expanded during brainstorming into a five-priority scope). A new tracking issue will roll #44 + the expanded scope; #44 closes on merge.
