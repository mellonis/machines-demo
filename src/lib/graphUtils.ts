 ...
 */
export function bareIdOf(id: number, graph: TuringGraph | null): number {
  if (!graph) return id;
 ...
 */
export function highlightExpand(id: number, graph: TuringGraph | null): number[] {
  if (!graph) return [id];
  const node = graph.nodes[id];
 ...
 * the input id.
 */
export function equivalentIds(id: number, graph: TuringGraph | null): number[] {
  if (!graph) return [id];
