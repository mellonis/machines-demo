...
import { applyHighlight, applyIndicator } from './applyHighlight.ts';
import { indexGraph } from './graphIndexes.ts';
import { recordingOps, type RecordedOp } from './highlightOps.ts';
...
function loadGraph(name: string): TuringGraph {
...
function run(highlight: GraphHighlight | null, graph: TuringGraph, prev: number | 'idle' | null = null): {
  ops: RecordedOp[];
