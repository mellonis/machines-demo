...
└── lib/
    ...
    │   ├── toolchainHelpers.ts   pure helpers: seed tapes ↔ wasm/glyph/snapshot conversions, `buildLineMap` (addr ↔ `{file, line}` via the program's `lineOf`, deliberately not `addressForLine`, which snaps forward), `resolveLoc` / `positionKey` (the `<file>:<fn>:<line>` step-granularity key a source-level Step compares), stdlib export index
    │   ├── toolchainHelpers.test.ts  Vitest suite for the pure helpers against the real wasm module — seeds / linemap / step-positions / layouts-equal / delta-clamp (cites T-delta-... / T-linemap-... / T-position-key... / T-layouts-... / T-seed-...)
...
- **Orchestrator**: `ToolchainView.svelte` (sibling of `MachineView.svelte`, which this work left untouched) over `ToolchainRunner` (`toolchainRunner.ts`) → `toolchainWorker.ts` → `ToolchainCore` (`workerCore.ts`, testable under Node against the real module via `testModule.ts`). Protocol in `src/lib/toolchain/types.ts`. Same five execution modes (MANUAL / RUNNING_AUTO / RUNNING_CONTINUOUS / RUNNING_PAUSED / HALTED); Step is one *source-level* step — `pump(1)` repeated until the resolved `(file, function, line)` position differs from where the step began, the line granularity of the toolchains' own debug adapter (a breakpoint, `debugger`, manual pause or the program ending partway ends it early and reports that instead; `TOOLCHAIN_SLICE_BUDGET` retirements cap a line that jumps to itself). TM-1 lowers one transition to several instructions, so `stats.steps` jumps by more than one per Step there. Auto-run is one source-level step per interval, continuous is `pump(TOOLCHAIN_SLICE_BUDGET)` slices with a `progress` heartbeat and an event-loop yield between slices. See `docs/execution-model.md` (toolchain engines).
